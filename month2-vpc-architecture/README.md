...
**Layer-by-Layer Security Implementation:** Starting with the outermost layer, the Internet Gateway provides the controlled entry point to the VPC. Unlike a traditional router that simply forwards traffic, I configured this gateway to work with route tables that explicitly define which subnets can receive internet traffic. Only the public subnet has a route to the Internet Gateway, meaning the private subnet is unreachable from the internet by design, not just by firewall rules.
